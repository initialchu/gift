...
测试阶段存入数据库的记录，在服务部署时会不会丢失？

## 25.2 答案：不会

**数据和应用代码是分离的。** 部署只更新应用代码，不会碰数据库文件。

```
┌─────────────────────────┐     ┌──────────────────────┐
│   应用代码 (Go/Vue)      │────▶│   MySQL 数据库        │
│   在 a:\gift\ 目录下     │     │   数据存在磁盘上       │
│                          │     │   (完全独立的位置)    │
│   部署 = 更新这里的文件   │     │                      │
│                          │     │   部署 ≠ 动这里       │
└─────────────────────────┘     └──────────────────────┘
...
本地 MySQL 直连，端口 **3307**。DSN 在 `server/config/config.yml` 中配置。

数据存在 MySQL 自己的数据目录（Windows 通常在 MySQL 安装目录下的 `data/` 文件夹），与 `a:\gift\` 项目目录完全独立。

## 25.4 GORM AutoMigrate 安全性

启动时调用的 `AutoMigrate` 只会：
- 创建不存在的表
- 添加缺失的列
- 添加缺失的索引

**永远不会**删除列、删除表、或清空数据。所以改了模型结构重启也不会丢数据。

## 25.5 什么情况下数据会丢

| 场景 | 安全性 | 说明 |
|------|--------|------|
| 重新编译 Go 后端 | ✅ 安全 | 代码变了，数据没变 |
| 重新构建前端 | ✅ 安全 | 前端跟数据库毫无关系 |
| 改了模型，重启后端 | ✅ 安全 | AutoMigrate 只加列不删 |
| 删了 MySQL 容器（Docker） | ❌ 危险 | 没有 volume 挂载数据就没了 |
| 手动 DROP DATABASE | ❌ 危险 | — |
| 重装 MySQL | ❌ 危险 | 卸载时可能清掉 data 目录 |
| 改了 DSN 指向另一个空库 | ❌ 危险 | 新库没有旧数据 |

## 25.6 Docker 部署注意事项（未来）

如果以后用 Docker 部署 MySQL，**必须加 volume 挂载**：
