  ...
  <div class="giftbook-detail">
    ...
    <el-card v-loading="loading">
      ...
      <div v-if="book" class="detail-info">
        <p>创建日期：{{ formatDate(book.event_date) }}</p>
        <p>创建者：{{ book.created_by }}</p>
      </div>

      <el-empty v-if="!loading && !book" description="账本不存在" />
    </el-card>
     ...
     <el-dialog v-model="dialogVisible" title="添加记录">
        <el-form :model="form">
          ...
          <el-form-item label="金额">
           ...
           <el-form-item label="去的时间">
           <el-date-picker v-model="form.gone_at" type="date" placeholder="选择日期" />
            </el-form-item>
    ...
    <div class="records-section">
      ...
      <GiftRecords ref="giftrecords" direction="去"/>
    </div>
  </div>
</template>
